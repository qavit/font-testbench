...
2. 啟動本機版做視覺比較。
   ...
   字體卡片是等高橫向長條列，文字區可以水平捲動；可以拖曳卡片左側標題區排序，也可以逐一移除；左側設定表單可收合。

   紅字表示該字型檔的 cmap 沒有對應 code point，瀏覽器需要 fallback。這個判斷需要 `scripts/serve.py` 開著，且字型卡片必須來自已掃描的本機字型檔路徑；純粹只靠系統 font-family 名稱或直接拖放進瀏覽器的字型，前端無法讀取 cmap。
